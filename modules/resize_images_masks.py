...
class ImageResizer:
    ...
    def __init__(
        ...
        images_dir: str,
        masks_dir: str,
        resized_images_dir: str,
        resized_masks_dir: str,
        image_size: int,
    ):
        ...
        # Number of new images/masks on the width and on the height of the original image/mask
    ...
    def cutting_mask(self, image_name: str, show=False):
        ...
        og_mask = data_reader.read_mask(
            os.path.join(self.masks_dir, image_name).replace(".png", "_mask.png")
        )
        ...
        width_shift = ((number_width_images * self.image_size) - width) / (
            number_width_images - 1
        )
        ...
        for height_idx in range(number_height_images):
            for width_idx in range(number_width_images):
                start_point_height = int(
                    (height_idx * self.image_size) - (height_shift * height_idx)
                )
                start_point_width = int(
                    (width_idx * self.image_size) - (width_shift * width_idx)
                )

                self.smaller_masks["values"].append(
                    og_mask[
                        start_point_height : start_point_height + self.image_size,
                        start_point_width : start_point_width + self.image_size,
                    ]
                )
                self.smaller_masks["coordinates"].append(
                    (start_point_height, start_point_width)
                )

        # Find the smaller mask that as the largest amout of parking spot
        size_of_parking = [
            sum(sum(self.smaller_masks.get("values")[i]))
            for i in range(len(self.smaller_masks.get("values")))
        ]
        max_parking_mask = np.argmax(size_of_parking)

        # Save the number of masks on the width and on the height inside the dictionnary.
        # Useful for later visual verifications with plt.
        ...
        if show:
            _, axs = plt.subplots(
                number_height_images, number_width_images, figsize=(8, 8)
            )
            for ax, mask, idx in zip(
                axs.ravel(),
    ...
    def best_masks(self, number_of_mask: int, show=False):
        ...
        for _ in range(number_of_mask):
            ...
            self.dict_best_masks["values"].append(
                self.smaller_masks.get("values")[max_mask_idx]
            )
            self.dict_best_masks["coordinates"].append(
                self.smaller_masks.get("coordinates")[max_mask_idx]
            )
        ...
        if show:
            _, axs = plt.subplots(
                self.number_width_height[0], self.number_width_height[1], figsize=(8, 8)
            )

            for ax, mask, parking_size in zip(
                axs.ravel(), self.smaller_masks.get("values"), list_of_parking_size
            ):
    ...
    def save_new_images_masks(self, number_of_mask: int, image_name: str, show=False):
        ...
        for mask_idx in range(len(self.dict_best_masks.get("coordinates"))):
            ...
            y_starting_point, x_starting_point = self.dict_best_masks.get(
                "coordinates"
            )[mask_idx]

            # Recreate the new square smaller corresponding image from those coordinates
            new_image = og_image[
                y_starting_point : y_starting_point + self.image_size,
            ...
            resized_mask_path = os.path.join(
                self.resized_masks_dir, image_name
            ).replace(".png", f"_{mask_idx+1}_mask.png")
            directory_manager.ensure_directory_exists(resized_mask_path)
            cv2.imwrite(resized_mask_path, self.dict_best_masks.get("values")[mask_idx])

            # Save the new corresponding image
            resized_image_path = os.path.join(
                self.resized_images_dir, image_name
            ).replace(".png", f"_{mask_idx+1}.png")
            directory_manager.ensure_directory_exists(resized_image_path)
            cv2.imwrite(resized_image_path, new_image)

        print(
            f'{image_name} correctly resized into {len(self.dict_best_masks.get("coordinates"))}',
...
if __name__ == "__main__":
    ...
    validation_image_names = list(
        np.random.choice(os.listdir(DATA_IMAGE_DIR), size=18, replace=False)
    )
    training_image_names = list(
        set(os.listdir(DATA_IMAGE_DIR)) - set(validation_image_names)
    )
    ...
    for image_name in training_image_names:
        training_image_resizer.save_new_images_masks(
            number_of_mask=4, image_name=image_name
        )
    ...
    for image_name in validation_image_names:
        validation_image_resizer.save_new_images_masks(
            number_of_mask=4, image_name=image_name
        )
